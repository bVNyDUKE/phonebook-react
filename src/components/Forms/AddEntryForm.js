...
const FormInputs = ({setContacts, open, setOpen}) => {
    ...
    function saveContact(){
        if(name.valid && surname.valid && phone.valid){
            setContacts( oldList => {
                const newContact = [{id: Number(oldList.length + 1), name: name.value, surname: surname.value, number:phone.value, category:category}]
    ...
    return(
    <div className='ui centered card' style={{display: !open ? 'none' : null}}>
            ...
            <div className='ui form'>
                ...
                <div className='inline fields'>
                        <div className="ui radio">
                            <input type='radio' value='Home' checked={category === "Home"} onChange={() => setCategory('Home')}/>
                            <label>Home <i className="home icon"></i></label>
                        </div>
                        <div className="ui radio">
                            <input type='radio' value='Work' checked={category === "Work"} onChange={() => setCategory('Work')}/>
                            <label>Work <i className="building icon"></i></label>
                        </div>
                </div>
...
const  AddEntryForm = ({setContacts}) => {
    ...
    return(
        <div className='ui basic content center aligned segment'>
            ...
            <FormInputs setContacts={setContacts} open={open} setOpen={setOpen}></FormInputs>
    </div>
    )
}
